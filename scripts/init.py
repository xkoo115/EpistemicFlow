    ...
    try:
        ...
        async with db_manager.engine.connect() as conn:
            ...
            result = await conn.execute(
                ...
                (session_id, workflow_name, current_stage, status, agent_state_json, metadata_json, created_at, updated_at)
                VALUES 
                (:session_id, :workflow_name, :current_stage, :status, :agent_state, :metadata, datetime('now'), datetime('now'))
                """),
                {
                    ...
                    "status": "pending",
                    "agent_state": '{"concept": "AI驱动的科研自动化", "keywords": ["AI", "科研", "自动化"], "progress": 0.1}',
                    "metadata": '{"creator": "system", "description": "示例研究工作流", "tags": ["sample", "test"]}',
                },
            )
            await conn.commit()
